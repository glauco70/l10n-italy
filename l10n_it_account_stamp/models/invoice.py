...
class account_invoice(Model):
    ...
    def compute_stamps(self, cr, uid, ids, context=None):
        ...
        for inv in self.browse(cr, uid, ids):
            ...
            for key in taxes.keys():
                tax_base_amounts[key[0]] = tax_base_amounts.get(key[0], 0.0
                    ) + taxes[key]['base_amount']
            to_apply_stamp_product_ids = set()
            stamp_product_ids = product_obj.search(cr, uid, [
                    ('is_stamp', '=', True),
                    ('company_id', '=', inv.company_id.id),
                ], context=context)
            invoice_line_to_unlink_ids = [l.id for l in inv.invoice_line \
                if l.product_id and l.product_id.is_stamp]
            invoice_line_obj.unlink(cr, uid, invoice_line_to_unlink_ids,
                context)
            for stamp in product_obj.browse(cr, uid, stamp_product_ids,
                    context):
                total_tax_base = 0.0
                for tax_code_id in tax_base_amounts.keys():
                    if tax_code_id in [t.id for t in \
                        stamp.stamp_apply_tax_code_ids]:
    ...
    def button_reset_taxes(self, cr, uid, ids, context=None):
        ...
        return super(account_invoice, self).button_reset_taxes(cr, uid, ids,
                context)
